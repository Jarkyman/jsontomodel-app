...
function generateClass(
  ...
  for (const key in jsonObject) {
    ...
    let rawType = getObjCType(rawValue);
    let type = rawType;

    // Custom class handling
    if (typeof rawValue === 'object' && rawValue !== null && !Array.isArray(rawValue)) {
      const nestedClassName = sanitizeIdentifier(toPascalCase(key));
      ...
      const nestedClassName = sanitizeIdentifier(toPascalCase(key.endsWith("s") ? key.slice(0, -1) : key));
