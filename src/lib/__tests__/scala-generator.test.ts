...
describe('generateScaleCode', () => {
    ...
    it('should generate correct Scala classes with default options', () => {
        ...
        expect(normGenerated).toContain("class user_data_profile( val theme: String )");
        
        // Check for main user class
        expect(normGenerated).toContain("class user_data( val user_id: Int, val user_name: String, val is_active: Boolean, val profile: Map, val tags: List )");
    });
    ...
    it('should generate without types if disabled', () => {
        ...
        const normGenerated = normalize(generated);
        expect(normGenerated).toContain('class simple( val id )');
    });
    ...
    it('should include default values if enabled', () => {
        ...
        const normGenerated = normalize(generated);
        expect(normGenerated).toContain("class item( val name: String = \"test\", val count: Int = 5 )");
    });
    ...
    it('should not use snake case if disabled', () => {
        ...
        const normGenerated = normalize(generated);
        expect(normGenerated).toContain('class UserData( val userName: String )');
    });

    it('should not generate a class body if includeStruct is false', () => {
        const options: ScaleGeneratorOptions = { ...defaultOptions, includeStruct: false };
